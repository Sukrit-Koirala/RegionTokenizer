#!/bin/bash
#SBATCH --job-name=region_lm_runs
...
REGION_MAP_DIR=runs/region_maps
...
# ── 0. Build region maps from trained baseline ────────────────────────────────
echo "=============================="
echo "Run 0/5: build_regions"
echo "=============================="
python train_region_lm.py \
    ...
    --region_vocab_size  5000 \
    --region_top_k       20 \
    --region_max_tokens  500000 \
    --n_clusters         50 \
    --leaf_min_size      30 \
    --seed               42 \
    --device             cuda
...
mkdir -p runs/coarse
python train_region_lm.py \
    --mode coarse --output_dir runs/coarse \
    "${COMMON_ARGS[@]}"
...
echo "=============================="
mkdir -p runs/oracle_coarse
python train_region_lm.py \
    --mode oracle_coarse --output_dir runs/oracle_coarse \
    "${COMMON_ARGS[@]}"
...
echo "=============================="
mkdir -p runs/coarse_leaf
python train_region_lm.py \
    --mode coarse_leaf --output_dir runs/coarse_leaf \
    "${COMMON_ARGS[@]}"
...
echo "=============================="
mkdir -p runs/random_control
python train_region_lm.py \
    --mode random_control --output_dir runs/random_control \
    "${COMMON_ARGS[@]}"
...
mkdir -p runs/oracle
python train_region_lm.py \
    --mode oracle --output_dir runs/oracle \
    "${COMMON_ARGS[@]}"
...
echo "=============================="
for run in coarse oracle_coarse coarse_leaf random_control oracle; do
