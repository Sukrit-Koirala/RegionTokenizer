#!/usr/bin/env python3
"""
train_local_detail_selector_v1c.py — Local Detail Selector V1C
Stage 2C: Candidate Selector + Edit Gate (Decoupled Architecture).
...
class LocalDetailSelectorGate(nn.Module):
    ...
    def __init__(self, token_emb_weight, tok_arr, reg_arr,
        ...
        # ── Selector (same architecture as V1/V1B) ──
    ...
    def gate_forward(self, cand_repr, gate_cand_idx, h_prime, base_lgts,
        ...
        sel_remb = self.region_emb(sel_reg.clamp(0, self.tok_arr.shape[0] - 1))
        ...
        if self.sr_enabled and self.super_emb is not None and cand_sups is not None:
            ...
            sel_semb = self.super_emb(sel_sup.clamp(0, rlen))
...
def write_report_v1c(run_dir, args, baselines, best_m_conservative,
    ...
    lines = ["# Local Detail Memory Selector V1C — Decoupled Selector + Gate\n"]
    ...
    lines.append("| Metric | Stage 1 | Stage 2 | Stage 2B | Stage 2C (this) |")
    lines.append("|--------|---------|---------|----------|-----------------|")
    lines.append(f"| ctg   | 0.040   | 0.054–0.063 | pending | {f(ar_ctg)} |")
    lines.append(f"| caw   | 0.062   | 0.079–0.094 | pending | {f(ar_caw)} |")
    lines.append(f"| fv_gain | -0.0199 | -0.0173 | pending | {f(fvg)} |")
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f" Local Detail Selector V1C complete.")
...
def _parse():
    p = argparse.ArgumentParser(
        description="Local Detail Selector V1C — Decoupled Selector + Gate")
    ...
    p.add_argument("--output_root",         default="runs/local_detail_selector_v1c")
    p.add_argument("--run_name",            default="cand_xattn_editgate_v1c")
    ...
    p.add_argument("--correctable_fraction",type=float, default=0.35)
    ...
    # V1C loss weights
    p.add_argument("--lambda_selector",         type=float, default=1.0)
    p.add_argument("--lambda_gate",             type=float, default=1.0)
    p.add_argument("--gate_pos_weight",         type=float, default=3.0)
    p.add_argument("--lambda_noharm_gate",      type=float, default=1.0)
